...
for file in $files; do
    ...
    # Java API接口地址
    api_url="http://tms-app-cdc:19201/ncbs-cgmdab-cl-loan-rlvc/sync"
    # 超时设置
...
EOF
    ...
    result=`curl -s $time_url -X POST -H "Content-Type: application/json" -d "$request_data" $api_url | grep '"success":true\|"code":"200"'|wc -l`
    
    # 调用完成时间
    echo `date "+%Y-%m-%d %H:%M:%S"` "执行完成，返回结果：result=$result" >> ${LOGFILE}
    
    # 返回结果中包含成功标识则为成功，否则失败
    if [ $result -eq 1 ]; then
        ...
        echo `date "+%Y-%m-%d %H:%M:%S"` "-----------检查-检查-检查!!!!!!-----" >> ${LOGFILE}
        echo `date "+%Y-%m-%d %H:%M:%S"` "数据导入失败，请检查Java服务状态" >> ${LOGFILE}
        echo `date "+%Y-%m-%d %H:%M:%S"` "脚本执行结束 失败" >> ${LOGFILE}
        fail='-1'
        echo $fail >> ${LOGFILE}
        echo $fail
