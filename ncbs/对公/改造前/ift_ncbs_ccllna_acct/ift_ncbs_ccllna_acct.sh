...
TABLE_COLUMNS=--columns=ln_due_bill_no,col_accru_outp_tax,rece_accru_outp_tax,cust_no,cust_name,biz_inst_no,accoun_inst_no,ln_prod_no,prod_name,contr_no,open_acct_dt,val_dt,exp_dt,mon_unit_ln_term,ln_days,ln_form,accru_non_accru_stat,ln_acct_stat,mul_ln_ded_sequ,mul_ln_overd_ded_sequ,curre,contr_amt,due_bill_amt,regu_pri,overd_pri,dead_ln_pri,bad_debt_pri,rece_accru_int,col_accru_int,rece_deb_int,col_deb_int,rece_accru_pena_int,col_accru_pena_int,rece_pena_int,col_pena_int,on_bal_accru_compou_int,on_bal_compou_int,off_bal_accru_compou_int,off_bal_compou_int,accru_subsi_on_int,rece_subsi_on_int,prep_int,wri_off_pri,wri_off_int,rece_cost,alr_wri_off_pri_int,wri_off_int_compou_int,last_fin_tran_day,chan_no,chan_name,ln_usa,due_bill_chara,ast_stat,notes_info,dtl_seri_no,open_acct_inst_no,open_acct_tell,canc_acct_tell,canc_acct_dt,bat_grp_no,rede_tm_poi_int_pay_off_ind,ln_wri_off_stat,by_reve_ind,col_accru_subsi_on_int,col_subsi_on_int,subsi_on_int_accru_compou_int,col_subsi_on_int_accru_compou_int,subsi_on_int_compou_int,col_subsi_on_int_compou_int,revol_fund_amt,revol_amt1,revol_amt2,rese_fie,rese_fie_1,rese_fie_2,mainte_tell,mainte_inst,mainte_dt,mainte_tm,record_stat
TARGET_FILE=tp_ncbs_${org_table_name}.txt

# 读取需要维护的借据号
ln_due_bill_no_all_file="ln_due_bill_no_all_file.txt"
ln_due_bill_no_all_sql="select ln_due_bill_no from ncbs_ccllna_acct union all select ln_due_bill_no from tp_ncbs_ccllnb_chk_write_off union all select ln_due_bill_no from tp_ncbs_ccllnb_non_perfmg_asset union all select ln_due_bill_no from tp_ncbs_cgmdab_cl_loan_rlvc;"

# 读取贷款属性表中要过滤掉的借据号
ln_due_bill_no_all_exclusion_file="ln_due_bill_no_all_exclusion_file.txt"
ln_due_bill_no_all_exclusion_sql="select ln_due_bill_no from tp_ncbs_ccllnb_basic_attr where ln_biz_clas in ('2', '3');"
