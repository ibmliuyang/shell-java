    ...
    public ResultVo sync(@RequestBody DataSyncRequest request) throws Exception {
        ...
        try {
            ...
            if (StrUtil.isBlank(request.getProcessDate())) {
                ...
                log.error("处理日期不能为空");
                throw new BizException("处理日期不能为空");
            }
            ...
            Long endTime = System.currentTimeMillis();
            Long consumingTime = endTime - startTime;
            SysInterfaceLogUtil.insertIFTLog("", request.getTableName(),interfaceName,JSON.toJSONString(request), JSON.toJSONString(errList), getLoginUser(),consumingTime);
