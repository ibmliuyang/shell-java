...
public class NcbsCgmdabClLoanRlvcController extends ControllerSupport {
    ...
    */
    ...
    @POST
    @Path("sync")
    @AspLog(title = "资产损失:临时表-抵债资产对公贷款关联表-数据同步", bizType = AuditOpBizType.UPDATE, isSaveRequestData = false)
    public ResultVo sync(@RequestBody DataSyncRequest request) throws Exception {
        Long startTime = System.currentTimeMillis();
        List<String> errList = new ArrayList<>();
        String interfaceName = "抵债资产对公贷款关联表";
        ...
        try {
            ...
            // 调用服务层处理数据同步
            DataSyncResult result = ncbsCgmdabClLoanRlvcService.syncData(request);
