...
#***************************************************************
#系统名称： 税务管理系统
#脚本名称： ift_ncbs_crllnb_basic_attr.sh
#脚本功能： 零售贷款属性表批量导入数据
#
#版本        日期       编写者    项目组/修改内容
#-------   -------     -------  ----------
...
# 表名
org_table_name="crllnb_basic_attr"
# 贷款业务分类所在列
ln_biz_clas_col=6
...
TABLE_COLUMNS=--columns=ln_due_bill_no,cust_no,ln_obj_type,ln_categ,ln_type_cd,ln_biz_clas,off_bal_prod_ind,der_ln_ind,der_biz_type,der_biz_no,stamp_duty_rate,stamp_duty_amt,ded_tax_stat,assoc_coll_ind,acct_ple_ind,subsi_on_int_ind,biz_tax_cha_to_value_add_tax_ind,cost_ind,inf_ind,agen_ind,use_lim_ind,fin_ind,permit_securi_ind,fin_manag_meth,spe_ln_type,inbank_biz_type,lc_no,cred_enh_ln_ind,ava_for_sale_prod_vers,revol_fund_amt,revol_amt1,revol_amt2,rese_fie,rese_fie_1,rese_fie_2,rel_biz_no,lab_name,mainte_tell,mainte_inst,mainte_dt,mainte_tm,record_stat,overa_sit_aff_stat,overa_sit_aff_seq_no,optim_lock_vers_no,sys_timest
...
for file in $files; do
    ...
    # 过滤数据
    echo `date "+%Y-%m-%d %H:%M:%S"` "过滤数据 ln_biz_clas in (2, 3)" >> ${LOGFILE}
    awk -F '\\|\\+\\|' -v ln_biz_clas_col="$ln_biz_clas_col" '$ln_biz_clas_col == 2 || $ln_biz_clas_col == 3' "$unzip_data_file" > "${work_dir}/temp.txt"
